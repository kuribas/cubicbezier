...
mpRest :: DPoint -> Parser (MetaPath Double)
mpRest p = do
...
pathRest :: DPoint -> Parser (Path Double)
pathRest p = do
  string ".."; spaces
...
joinEq (JoinLine _) (JoinLine _) = True
joinEq (JoinCurve a b) (JoinCurve c d) =
  pointEq a c && pointEq b d
joinEq _ _ = True

pathEq :: Path Double -> Path Double -> Bool
